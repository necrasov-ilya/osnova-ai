...
Базовая ставка: Android + CameraX + ML Kit для дешёвого on-device анализа, Tesseract `rus+eng` для русского OCR, Gemma через LiteRT-LM для Markdown-сборки и строгий gate между ними.
...
На продуктовой стадии Gemma4-E2B не блокирует разработку. Архитектура должна позволять заменить on-device VLM на серверный fallback или мок, пока UI, очередь и Smart Insert набирают форму. В текущем Android MVP Gemma обязательна как рабочий путь, а OCR fallback нужен для устойчивости.
...
| Чем читать кириллицу | Tesseract `rus+eng` на стабильном cropped/выбранном кадре |
...
| Чем делать Markdown-сборку в MVP | Gemma 4 E2B `.litertlm` через LiteRT-LM |
| Как стримить ответ | `Conversation.sendMessageAsync(...)` -> `delta`-события |
| Чем рендерить Markdown | Markwon в нативном `TextView` |
| Чем делать сборку заметки | LLM по стабильным OCR-событиям, не по каждому кадру |
...
| На чём тестировать первым | Vivo X200 Ultra, Snapdragon 8 Elite |
...
| OCR | Быстро вытащить печатный текст | Когда кадр похож на текст | ML Kit Text Recognition + Tesseract `rus+eng` |
| Draft Builder | Очистить строки, убрать шум, собрать Markdown-кандидат | После OCR | собственные правила, confidence/quality score |
| Gemma | Собрать Markdown-конспект | Только после gate и draft builder | Gemma E2B через LiteRT-LM, серверный fallback только для разработки |
| Note engine | Понять, куда вставлять блок | При каждом готовом черновике | собственная логика + LLM с контекстом заметки |
| Markdown UI | Показать и отредактировать блок | В редакторе | NoteBlock + Markwon + edit sheet |
...
6. Tesseract `rus+eng` для кириллицы.
7. Gemma E2B через LiteRT-LM как главный on-device слой Markdown-сборки.
8. Потоковые `delta`-события в generating `NoteBlock`.
9. Markwon preview и edit sheet для исходного Markdown.
10. Android Thermal API как сигнал для будущего автоматического понижения режима.
